...
//De momento no las usamos porque no sabemos lo que hacen
...
-(NSArray *) executeSearchWithRequest: (NSFetchRequest*) request
    ...
    if (!_context) {
        ...
        res = [self.context executeFetchRequest:request
                                          error:&err];
        if (res == nil) {
            //la cagamos, ejecutamos el bloque que nos han pasado
            if (errorBlock != nil){
                errorBlock(err);
            }
        }
